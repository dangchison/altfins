...
class TelegramNotifier(BaseNotifier):
    ...
    def __init__(self, token: str, chat_ids_map: dict[str, str]) -> None:
        """
        Args:
            token:    Telegram bot token.
            chat_ids_map: Dictionary mapping source_type to chat_id.
        """
        self._token = token
        self._chat_ids_map = chat_ids_map
    ...
    def send(self, setup: TradeSetup) -> None:
        """Send message to the specific chat ID based on its source type."""
        chat_id = self._chat_ids_map.get(setup.source_type)
        
        if not chat_id:
            log.warning("No chat ID configured for source type %s. Skipping Telegram alert.", setup.source_type)
            return
